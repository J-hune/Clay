...
void GLViewport::setOrbitDistance(const float d) {
    ...
    if (!qFuzzyCompare(old, m_cameraController.orbitDistance())) {
...
void GLViewport::generateTerrain() {
    ...
    if (m_terrainMode == 0) {
        // Mode flat
        TerrainBuilder::buildFlat(m_terrainMesh, res, res);
    } else if (m_terrainMode == 1) {
        // Mode heightmap
        const QImage img = TerrainBuilder::loadHeightmapFromUrl(m_heightmapSource);
        TerrainBuilder::buildFromHeightmap(m_terrainMesh, img, res, res, m_heightScale);
    }
...
void GLViewport::computeMouseNDC(const QPointF &localPos) {
    ...
    if (w_width > 0.f && w_height > 0.f) {
        const QVector2D newNDC(
            (2.0f * localPos.x() / w_width) - 1.0f,
            (2.0f * localPos.y() / w_height) - 1.0f
        );
...
void GLViewport::forwardMouseToCamera(QMouseEvent *event) {
...
void GLViewport::forwardKeyboardToCamera(QKeyEvent *event, bool isPress) {
    if (isPress) {
...
void GLViewport::keyPressEvent(QKeyEvent *event) {
    forwardKeyboardToCamera(event, true);
    update();
}

void GLViewport::keyReleaseEvent(QKeyEvent *event) {
    forwardKeyboardToCamera(event, false);
    update();
}
...
void GLViewport::mouseMoveEvent(QMouseEvent *event) {
    ...
    forwardMouseToCamera(event);
    
    if (!qFuzzyCompare(oldYaw, m_cameraController.camera().yaw())) {
        emit yawChanged();
    }
    if (!qFuzzyCompare(oldPitch, m_cameraController.camera().pitch())) {
        emit pitchChanged();
    }
...
void GLViewport::hoverMoveEvent(QHoverEvent *event) {
    ...
    computeMouseNDC(localPosF);
    
    if (m_raycastRequested) {
        update();
    }
}
